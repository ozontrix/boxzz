...
export default function CartPage() {
  ...
  const handleQuantityChange = (productId: string, delta: number, currentQty: number, moq: number) => {
              ...
              <AnimatePresence>
                {confirmClear && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 8 }}
                    className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl border border-zinc-200 shadow-lg p-3 z-10"
                  >
                    <p className="text-xs text-zinc-600 mb-2">Clear all items?</p>
            ...
            <div className="lg:col-span-2 space-y-3">
              <AnimatePresence>
                {items.map((item, idx) => (
                  ...
                  >
                    {/* Image */}
                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-gradient-to-br from-zinc-50 to-zinc-100 flex items-center justify-center text-2xl sm:text-3xl shrink-0 border border-zinc-100">
                      {item.image || "📦"}
                    </div>
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <div className="flex items-center gap-2">
                          <div className="flex items-center bg-zinc-50 border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
                            <motion.button
                              whileTap={{ scale: 0.9 }}
                              onClick={() => handleQuantityChange(item.productId, -1, item.quantity, 1)}
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-32 bg-white rounded-xl border border-zinc-100 p-4 sm:p-5 shadow-sm">
                ...
                {shipping > 0 && (
                  <div className="mt-3 p-3 bg-amber-50 rounded-lg border border-amber-100">
                    <div className="flex items-center gap-1.5 text-xs text-amber-700 mb-2">
                      <Truck className="w-3.5 h-3.5" />
                      <span>Add {formatPrice(2499 - subtotal)} more for FREE shipping</span>
                    </div>
                ...
                <div className="mt-4 pt-4 border-t border-zinc-100">
                  ...
                  <p className="text-xs text-zinc-500 mt-1">Inclusive of GST</p>
                </div>
